  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8">
      <div className="w-full max-w-md rounded-lg border p-8 shadow-sm">
        {user.avatarUrl && <img src={user.avatarUrl} alt={user.name} className="mx-auto mb-4 h-20 w-20 rounded-full" />}
        ...
        <div className="flex flex-col gap-3">
          <Link href="/profile/edit" className="rounded bg-blue-600 px-4 py-2 text-center text-white hover:bg-blue-700">
            Edit Profile
          </Link>
          <a href="/auth/logout" className="rounded border px-4 py-2 text-center text-red-600 hover:bg-red-50">
            Sign Out
          </a>
        </div>
      </div>
